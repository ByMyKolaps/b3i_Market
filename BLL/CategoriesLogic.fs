...
module CategoriesLogic =
    ...
    let UpdateCategoryPhoto (name:string, photo:string)=
        productRepo.UpdateCategoryPhoto(name, photo)
        

        
    let GetCategoryByName (text:string)=
        let answer = productRepo.GetCategoryByName text
        answer
    ...
    let DeleteCategory(categoryName:string) =
        ...
        let categoryProductsWithPhoto = productRepo.IFthisPhoroUseMoreProduct(category.Result.PhotoPath);

        match deleteCategory with
        | Success s ->
            if categoryProductsWithPhoto = false
                then 
