...
namespace DAL.Repositories
{
    public class ProductRepo : Repo
    {
        ...
        {
            IMongoCollection<Product> collection = ProductConnect();
            IMongoCollection<Category> collectionCat = ConnectCategory();
            var filter = new BsonDocument("PhotoPath", path);
            var result1 = collection.Find(filter).ToList();
            var result2 = collectionCat.Find(filter).ToList();
            var result = result1.Count + result2.Count;
            if (result >= 1)
                return true;
            else
                return false;
        ...
        public UpdateResult UpdateCategoryPhoto(string name, string photo)
        {
            IMongoCollection<Category> collection = ConnectCategory();
            var filter = new BsonDocument("Name", name);
            var updatePhoto = Builders<Category>.Update.Set("PhotoPath",photo);
            return collection.UpdateOne(filter, updatePhoto);
        }
