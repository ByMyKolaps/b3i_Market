...
namespace B3I_Market.Controllers
{
    public class ProductController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> EditCategory(string categoryName)
        {
            var result = await CategoriesLogic.GetCategoryByName(categoryName);
            return View(result);
        }

        [HttpPost]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> EditCategory(string _categoryName, IFormFile photo)
        {

            var result = await CategoriesLogic.GetCategoryByName(_categoryName);
            try
            {
                if (photo != null)
                {
                    string photoPath = await _environment.AddFile(photo, "/img/");
                    if (result.PhotoPath != "/img/no-image.png")
                        if (!BLL.ProductLogic.IFthisPhoroUseMoreProduct(result.PhotoPath))
                            _environment.DeletePhoto(result.PhotoPath);
                    var ans = CategoriesLogic.UpdateCategoryPhoto(_categoryName, photoPath);
                    if (ans.IsAcknowledged)
                        TempData.Add("Success", "Photo has been changed");
                }
            }
            catch
            {
        ...
        public async Task<IActionResult> EditProductForm(Product product, IFormFile photo,string _id,string _photoPath)
        {
            ...
            try
            {
                ...
                {
                    photoPath = await _environment.AddFile(photo, "/img/");
                    if (photoPath != _photoPath && _photoPath!= "/img/no-image.png")
